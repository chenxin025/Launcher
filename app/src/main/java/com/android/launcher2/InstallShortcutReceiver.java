...
//        	if (!isSystemShortcut(context, intent)) {
//        		return;
//        	}
        	// M: End
     ...
     * M: Added by liudekuan
     */
    public static boolean isSystemShortcut (Context context, Intent intent) {
    	
    	if (LauncherLog.DEBUG) {
    		LauncherLog.d(TAG, "isSystemShortcut: intent = " + intent);
    	}
    	...
    	if (action.equals("com.android.contacts.action.QUICK_CONTACT")) {
    		return true;
    	}
    ...
    private static void installShortcutArray(Context context, Intent data) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            // M: Added by liudekuan
            if (!isSystemShortcut(context, (Intent)intentArray[i])) {
            	continue;
            }
            // M: End
    ...
    private static void processInstallShortcut(Context context,
        ...
        synchronized (app) {
            ...
            int screenCount = LauncherApplication.getDisplayFactory(context).getScreenCount(context);
            for (int i = 0; i < (2 * screenCount) + 1 && !found; ++i) {
                ...
                if (0 <= si && si < screenCount) {
                    ...
                    /// M: Added by liudekuan
                    if (result[0] == INSTALL_SHORTCUT_ADD_FAIL) {
                    	break;
                    }
                    /// M: End
        ...
        /// M: Added by liudekuan
        else if (result[0] == INSTALL_SHORTCUT_ADD_FAIL) {
        	Toast.makeText(context, context.getString(R.string.shortcut_installed_fail), Toast.LENGTH_SHORT).show();
        }
        /// M: End
        ...
        for (int j = 0; j < dataArray.length; j++) {
            ...
            synchronized (app) {
                ...
                for (int i = 0; i < (2 * Launcher.SCREEN_COUNT) + 1 && !found; ++i) {
                    ...
                    if (0 <= si && si < LauncherApplication.getDisplayFactory(context).getScreenCount(context)) {
                    /// M: End
                        found = installShortcut(context, data, items, name, intent, si, exists, sp,
                                result);
                        if (found) {
    ...
    private static boolean installShortcut(Context context, Intent data, ArrayList<ItemInfo> items,
            ...
            if (intent != null) {
                ...
                if (duplicate || !shortcutExists) {
                    ...
                    ShortcutInfo info = app.getModel().addShortcut(context, data,
                            ...
                            tmpCoordinates[0], tmpCoordinates[1], false);
        ...
        /// M: Added by liudekuan
        /// R: After installing, set sInstallingShortcut false
    ...
    private static boolean findEmptyCell(Context context, ArrayList<ItemInfo> items, int[] xy,
        ...
        for (int i = 0; i < items.size(); ++i) {
            ...
            if (item.container == LauncherSettings.Favorites.CONTAINER_DESKTOP) {
                if (LauncherModel.getActualScreen(item.screen, item.container) == screen) {
                    cellX = item.cellX;
                    cellY = item.cellY;
                    spanX = item.spanX;
                    spanY = item.spanY;
