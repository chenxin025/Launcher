...
public class CellLayout extends ViewGroup {
    ...
    public static int mFreeSpace = 0;

    public CellLayout(Context context) {
        this(context, null);
        mFreeSpace = (int) this.getResources().getDimension(R.dimen.cell_freespace);
    }
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        //M:by chenxin under edit mode  set backgroud bitmap
        //mNormalBackground = res.getDrawable(R.drawable.homescreen_blue_normal_holo);
        mNormalBackground = res.getDrawable(R.drawable.homescreen_blue_strong_holo);
		...
		int[] params = LauncherApplication.getDisplayFactory(getContext())
				.getCellLayoutParams();

//		mCellWidth = params[0];
//		mCellHeight = params[1];
		
		//added by shanlijuan
        mCellWidth = LauncherApplication.getDisplayFactory(context).getWorkspaceCellWidth();
        mCellHeight = LauncherApplication.getDisplayFactory(context).getWorkspaceCellHeight();
        //end
    ...
    public boolean addViewToCellLayout(View child, int index, int childId, LayoutParams params,
        ...
        //deleted by ETON guolinan,we do not need to scale the child`s size ,just keep its original size.
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // Modifid by liukaibang, {{ 
        // Gap need'n to caculate again..
        /* Old Code
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child.getVisibility() != GONE) {
                ...
                if(child instanceof Button){
                	int addViewWidth = child.getBackground().getIntrinsicWidth();
        			int addViewHeight = child.getBackground().getIntrinsicHeight();
        		 	lp.width = addViewWidth;
                	lp.height = addViewHeight;
                	 if(child.getId() == Workspace.ADD_BUTTON_ID){
	        			lp.x = (getWidth()- addViewWidth)/2;
	        			lp.y =(getHeight()- addViewHeight)/2;
	                }else  if(child.getId() == Workspace.DELETE_BUTTON_ID){
	                	lp.x= getWidth() - lp.width * 3 /2;
	                	lp.y = lp.height*3/4 ;
	                }
                //end
                int childLeft = lp.x;
    ...
    int[] findNearestArea(int pixelX, int pixelY, int minSpanX, int minSpanY, int spanX, int spanY,
        ...
        // added by liukaibang  {{ 
        // Fixed: Widget can be dragged to a full screen in 3x3 mode
        // default value -1, -1 indicate no suitable location found
        bestXY[0] = bestXY[1] = -1;
        // End }}
    ...
    private class ViewCluster {
        ...
        void computeEdge(int which, int[] edge) {
            ...
            // added by liukaibang. {{ 
            // We must chech edge's length here.
                ...
                switch (which) {
                    case LEFT:
                        ...
                        for (int j = cs.y; j < cs.y + cs.spanY; j++) {
                            if (j < length && (left < edge[j] || edge[j] < 0)) {
                                edge[j] = left;
                            }
                        }
                        break;
                    ...
                    case TOP:
                        ...
                        for (int j = cs.x; j < cs.x + cs.spanX; j++) {
                            if (j < length && (top < edge[j] || edge[j] < 0)) {
                                edge[j] = top;
                            }
                        }
                        break;
            ...
            switch (whichEdge) {
                case LEFT:
                    for (int i = cs.y; i < cs.y + cs.spanY; i++) {
                        if (i < length && edge[i] == cs.x + cs.spanX) {
                            return true;
                        }
                    }
                    break;
                ...
                case TOP:
                    for (int i = cs.x; i < cs.x + cs.spanX; i++) {
                        if (i < length && edge[i] == cs.y + cs.spanY) {
                            return true;
                        }
                    }
                    break;
    ...
    */
     ...
     */
	static boolean findVacantCells(int[] cellXY, int spanX, int spanY,
			int xCount, int yCount, boolean occupied[][]) {
        // mark space take by ignoreView as available (method checks if ignoreView is null)

		boolean foundCell = false;
		int startX = 0;
		int endX = xCount - (spanX - 1);
		int startY = 0;
		int endY = yCount - (spanY - 1);
...
out:            for (int i = x; i < x + spanX - 1 && x < xCount; i++) {
     ...
     * M: Added by liudekuan
     */
    public void markCellsAsOccupied (int cellX, int cellY, int spanX, int spanY) {
    	if (cellX < 0 || cellY < 0) return;
    	for (int x = cellX; x < cellX + spanX && x < mCountX; x++) {
            for (int y = cellY; y < cellY + spanY && y < mCountY; y++) {
                mOccupied[x][y] = true;
    ...
    public static class LayoutParams extends ViewGroup.MarginLayoutParams {
        ...
        public void setup(int cellWidth, int cellHeight, int widthGap, int heightGap,String name) {
            if (isLockedToGrid) {
                ...
                // modify by ygxing
//                int freeSpace = mScreenWidth - LauncherModel.getCellCountX() * cellWidth - (LauncherModel.getCellCountX() - 1) * widthGap;
//                if(cellWidth <= 0 || widthGap <= 0 || LauncherModel.getCellCountX() <= 0){
...
//                }
           ...
           Log.v("setup", "---isLockedToGrid--=" +isLockedToGrid
                + "---width--=" +width
                + "---height--=" +height
                + "---x--=" +x
                + "---y--=" +y
    ...
    //added by ETON guolinan,for when in springMode,add button adding a new screen to workspace.
    public void addButtonDrawable(int resourcesId, int id,Context context,View.OnClickListener onClickListener){
    	if (mLauncher.getWorkspace().getScreenEditState() && getChildCount() == 1 && getShortcutsAndWidgets().getChildCount() == 0){
	    	Button addButton = new Button(context);
			addButton.setBackgroundResource(resourcesId);
    ...
    public void deleteButtonDrawable(int buttonId){
    	for(int i = 0; i <= getChildCount() - 1; i++){
    		View  child = getChildAt(i);
    		if(child != null && child.getId() == buttonId){
