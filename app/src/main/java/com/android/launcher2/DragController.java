...
public class DragController {
    public static final String TAG = "Launcher.DragController";
    ...
    public static final boolean DragTwinklingAnimationEnable = true;
    public static final boolean DragBeatCompensationEnable = false;
    ...
    //added by chenxin
    public void setOriginatorView(View view){
    	mOriginator = view;
    }
    ...
    public void startDrag(Bitmap b, int dragLayerX, int dragLayerY,
        ...
        //>>added by wanghenan;for dragView Animation; 20140310-104400;
        if (DragTwinklingAnimationEnable) {
        	 mDragObject.dragView.startAnimation(mAnimationRecycleSet);
		}
        //<<added end 20140310-104400;
...
//<<added end 20140310-104400;              	
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if(!mLauncher.getWorkspace().getScreenEditState() && !mLauncher.mGaussOver 
            &&(time-mLauncher.mGaussBeginTime < Launcher.CLICK_INTERVAL_TIME))
        {
            Log.i("Launcher_test", "Launcher-onInterceptTouchEvent: (time = " +time+") - (mGaussBeginTime ="+mLauncher.mGaussBeginTime+")="+(time-mLauncher.mGaussBeginTime)+"  <1s");
            return true;
        }
        
        if(!mLauncher.getWorkspace().getScreenEditState() && !mLauncher.mGaussOver)
        {
            mLauncher.mGaussOver = true;
        }
...
//		}
    ...
    public boolean onTouchEvent(MotionEvent ev) { 
        ...
        case MotionEvent.ACTION_MOVE:
        	//>>added by wanghenan;
        	/*  workspace --> drag for moving;  Compensate Status Bar Height                 */
        	if (DragBeatCompensationEnable && mDragObject.dragSource instanceof Workspace) {
        		handleMoveEvent(dragLayerX, dragLayerY - Launcher.mStatusBarHeight);
    		} else {
        ...
        case MotionEvent.ACTION_UP:
            ...
            if (mDragging) {
                ...
                if (vec != null) {
                	...
                	//>>added by wanghenan;
                	/*  workspace --> drag  end; for two APP Merger into folder fail ;                */
                	if (DragBeatCompensationEnable && mDragObject.dragSource instanceof Workspace) {
                		 drop(dragLayerX, dragLayerY - Launcher.mStatusBarHeight);
            		}else {
            			drop(dragLayerX, dragLayerY);
        			}
                	//<< added end
    ...
    private void drop(float x, float y) {
        ...
        boolean accepted = false;
        if (dropTarget != null) {
            mLauncher.getHotseat().getLayout().mShortcutsAndWidgets.setDragView(null);
...
//>>added by wanghenan;for dragView Animation; 20140310-104400;
	   private static   AnimationSet mAnimationRecycleSet = new AnimationSet(true);

	   private static AlphaAnimation	mAlphaRecycleAnimation = new AlphaAnimation(1.0f, 0.5f);
		static
		{
			
			mAlphaRecycleAnimation.setDuration(500);
			mAlphaRecycleAnimation.setFillAfter(true);
			mAlphaRecycleAnimation.setRepeatMode(Animation.REVERSE);
			mAlphaRecycleAnimation.setRepeatCount(Animation.INFINITE);
			
//			mScaleRecycleAnimation.setDuration(500);
//			mScaleRecycleAnimation.setFillAfter(true);
//			mScaleRecycleAnimation.setRepeatMode(Animation.REVERSE);
//			mScaleRecycleAnimation.setRepeatCount(Animation.INFINITE);
	
			
	        mAnimationRecycleSet.addAnimation(mAlphaRecycleAnimation);
//	        mAnimationRecycleSet.addAnimation(mScaleRecycleAnimation);
		}
		
		public static AnimationSet getDragIconRecycleAnimationSet() {
			return mAnimationRecycleSet;
		}
//<<added end 20140310-104400;  
